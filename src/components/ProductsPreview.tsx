...
export function ProductsPreview({
  ...
  return (
    <section className={`max-w-[1200px] mx-auto px-4 sm:px-6 py-12 sm:py-16 ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6 sm:mb-8">
        <div className="flex-1">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#163A5F] mb-2 tracking-tight">
            {title}
          </h2>
      ...
      <div
        ref={scrollRef}
        className="flex gap-3 sm:gap-5 overflow-x-auto scroll-smooth snap-x snap-mandatory pb-4 -mx-4 px-4 sm:mx-0 sm:px-0"
      ...
      >
        ...
        <div className="products-carousel" style={{ display: 'contents' }}>
          {items.map((kassa) => {
            ...
            return (
              <Link
                key={kassa.id}
                href="/katalog-kass"
                className="group snap-start shrink-0 w-[240px] sm:w-[260px] bg-white rounded-xl border border-slate-200 overflow-hidden transition-all duration-200 hover:shadow-lg hover:border-[#163A5F]/30 hover:-translate-y-0.5 flex flex-col"
              >
                <div className="relative bg-slate-50 aspect-[4/3] flex items-center justify-center p-4 border-b border-slate-100">
                  ...
                  {mainImage && (
                    <Image
                      ...
                      width={140}
                      height={100}
                      className="max-h-28 w-auto object-contain opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-300"
                      unoptimized
                    />
                  )}
                  {kassa.hasAcquiring && (
                ...
                <div className="p-3 sm:p-4 flex-1 flex flex-col">
                  ...
                  <h3 className="text-sm sm:text-base font-bold text-[#163A5F] leading-tight mb-1.5 group-hover:text-[#1E4A78]">
                    {kassa.name}
                  </h3>
                  ...
                  <div className="mt-auto">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      <span className="text-base sm:text-lg font-bold text-[#163A5F]">
                    ...
                    {kassa.gift && (
                      <div className="mt-1.5 flex items-start gap-1 text-[10px] text-emerald-700 font-medium leading-tight">
          ...
          <Link
            href="/katalog-kass"
            className="snap-start shrink-0 w-[200px] sm:w-[220px] bg-gradient-to-br from-[#163A5F] to-[#1E4A78] rounded-xl overflow-hidden flex flex-col items-center justify-center p-6 text-white hover:shadow-lg transition-all hover:-translate-y-0.5"
          >
            <div className="w-14 h-14 rounded-full bg-white/15 flex items-center justify-center mb-3">
              <ArrowRight className="w-7 h-7" />
            </div>
            <h3 className="text-base font-bold text-center mb-1">Все модели</h3>
            <p className="text-xs text-white/70 text-center leading-snug">
              13 касс в каталоге с установкой под ключ
            </p>
          </Link>
        </div>
      </div>

      <div className="mt-3 sm:hidden text-center text-xs text-slate-400">
