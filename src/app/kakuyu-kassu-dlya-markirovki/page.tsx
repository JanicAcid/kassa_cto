  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
      text: 'Касса должна поддерживать формат фискальных документов (ФФД) версии 1.2 и работать с фискальным накопителем ФН-1.2 или новее. Это обязательное требование для передачи данных о маркированных товарах в Честный ЗНАК через ОФД. Без ФФД 1.2 пробитие чека по маркированному товару невозможно.',
      },
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'В большинстве случаев старую кассу можно адаптировать. Если оборудование поддерживает обновление прошивки до ФФД 1.2 — достаточно обновить ПО и заменить фискальный накопитель на ФН-1.2. Если модель не поддерживает ФФД 1.2 даже после обновления — придётся заменить кассу.',
      },
    },
    {
      '@type': 'Question',
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'Для небольшого магазина с небольшим потоком клиентов подойдут смарт-терминалы: Эвотор 6, AQSI 5Ф или Меркурий 185Ф. Они компактные, имеют сенсорный экран. Стоимость — от 8 800 ₽. Для магазина с одним кассовым местом этого достаточно.',
      },
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'Бюджетные мобильные кассы (Меркурий 185Ф) — от 8 800 ₽. Смарт-терминалы среднего сегмента (Эвотор 6, AQSI 5Ф) — от 18 650 до 20 000 ₽. Флагманские решения (Эвотор 7.3, Эвотор 10) — от 33 600 до 38 700 ₽. К цене кассы добавьте стоимость ФН (14 200–21 000 ₽) и настройку (от 3 000 ₽).',
      },
    },
    {
      '@type': 'Question',
...
const REQUIREMENTS = [
  ...
  },
  {
    ...
    title: 'Фискальный накопитель ФН-1.2 и новее',
    desc: 'Фискальный накопитель (ФН) — это чип внутри кассы, который хранит информацию о всех пробитых чеках. Для маркировки требуется модель ФН-1.2 или выше, так как только они поддерживают ФФД 1.2. Старые накопители (ФН 1.0, ФН 1.1) не подходят — их придётся заменить. Срок службы ФН — 15 или 36 месяцев, в зависимости от режима работы.',
  },
  {
...
const CASSES_BY_CATEGORY = [
  {
    ...
    models: [
      ...
      },
      {
        ...
        vendor: 'Инкотекс',
        price: '8 800 ₽',
        features: ['Сенсорный экран', 'Встроенный 2D-сканер', 'ФН-1.2, ФФД 1.2', 'Wi-Fi, USB'],
    ...
    models: [
      {
        ...
        vendor: 'АТОЛ',
        price: '21 450 ₽',
        features: ['Автоотрезчик чеков', 'USB, Ethernet, Wi-Fi', 'ФН-1.2, ФФД 1.2', '80 мм лента'],
      ...
      },
      {
        ...
        vendor: 'АТОЛ',
        price: '30 980 ₽',
        features: ['Надёжный механизм печати', 'USB, COM, Ethernet', 'ФН-1.2, ФФД 1.2', 'Тихая печать'],
      ...
      },
      {
        ...
        vendor: 'АТОЛ',
        price: '20 350 ₽',
        features: ['Компактный корпус', 'USB, Bluetooth', 'ФН-1.2, ФФД 1.2', '58 мм лента'],
      ...
      },
      {
        ...
        vendor: 'АТОЛ',
        price: '28 950 ₽',
        features: ['Высокая скорость печати', 'USB, Ethernet, Wi-Fi', 'ФН-1.2, ФФД 1.2', 'Надёжная конструкция'],
    ...
    models: [
      {
        ...
        vendor: 'Инкотекс',
        price: '8 800 ₽',
        features: ['Аккумулятор, портативность', 'Bluetooth, USB', 'ФН-1.2, ФФД 1.2', '58 мм лента'],
        best: 'Курьер, выездная торговля, ярмарка',
      },
      {
        name: 'AQSI 6Ф (с эквайрингом)',
        vendor: 'Аквариус',
        price: '20 000 ₽',
        features: ['Аккумулятор, портативная', 'Bluetooth, USB, Wi-Fi', 'ФН-1.2, ФФД 1.2', '58 мм лента'],
        best: 'Курьерская доставка, выездные продажи',
      },
      {
        name: 'AQSI 5Ф (с эквайрингом)',
        vendor: 'Аквариус',
        price: '22 100 ₽',
      ...
      },
      {
        ...
        vendor: 'ЭВОТОР',
        price: '18 900 ₽',
        features: ['Аккумулятор, компактный', 'Bluetooth, USB', 'ФН-1.2, ФФД 1.2', 'Низкая цена'],
...
const FAQS = [
  {
    ...
    a: 'Касса обязана поддерживать формат фискальных документов (ФФД) версии 1.2 и работать с фискальным накопителем ФН-1.2 или новее. Именно через ФФД 1.2 касса передаёт коды маркировки Data Matrix в ОФД, а оттуда — в Честный ЗНАК и ФНС. Без этого пробитие чека по маркированному товару невозможно. За использование кассы, не соответствующей требованиям, предусмотрены штрафы по ч. 4 ст. 14.5 КоАП РФ: для юрлиц — от 5 000 до 10 000 ₽, для должностных лиц — от 1 500 до 3 000 ₽.',
  },
  {
    q: 'Можно ли использовать старую кассу для маркировки?',
    a: 'В большинстве случаев старую кассу можно адаптировать под маркировку. Если модель аппаратно поддерживает ФФД 1.2 — достаточно обновить прошивку и заменить фискальный накопитель на ФН-1.2. Многие модели Атол, Штрих-М, Меркурий и Эвотор поддерживают обновление ПО. Если же касса не поддерживает ФФД 1.2 даже после обновления — придётся заменить оборудование. Мы бесплатно проконсультируем по совместимости вашей кассы: <Link href="tel:+78124659457" className="text-[#163A5F] underline underline-offset-2 hover:text-[#1E4A78]">+7 (812) 465-94-57</Link>.',
  },
  {
    q: 'Нужен ли сканер штрихкодов для работы с маркировкой?',
    a: 'Да, 2D-сканер для считывания кодов Data Matrix обязателен. Обычный линейный (1D) сканер для штрихкодов EAN-13 не сможет прочитать квадратный код маркировки. Некоторые кассы имеют встроенный 2D-сканер (Эвотор 7.3, AQSI 5Ф, Меркурий 185Ф), другие требуют подключения внешнего сканера через USB или Bluetooth. Внешний 2D-сканер — 4 300 ₽ (проводной) или 5 600 ₽ (Bluetooth).',
  },
  {
    q: 'Какая касса лучше для малого магазина?',
    a: 'Для магазина с 1–2 кассовыми местами и небольшим ассортиментом оптимальны смарт-терминалы: Меркурий 185Ф (8 800 ₽) — бюджетное решение, AQSI 5Ф (18 650 ₽) — сенсорный экран, Эвотор 7.3 (33 600 ₽) — флагман с большим дисплеем и мощным процессором. Все три модели работают на Android, поддерживают магазин приложений и легко настраиваются под маркировку.',
  },
  {
    q: 'Сколько стоит касса с поддержкой маркировки?',
    a: 'Бюджетные модели (Меркурий 185Ф) — от 8 800 ₽. Смарт-терминалы среднего сегмента (Эвотор 6, AQSI 5Ф) — от 18 650 до 20 000 ₽. Флагманские решения (Эвотор 7.3, Эвотор 10) — от 33 600 до 38 700 ₽. К стоимости кассы добавьте: фискальный накопитель (14 200–21 000 ₽), настройку под ключ (3 000 ₽) и при необходимости 2D-сканер (4 300 ₽). Рассчитайте точную стоимость в нашем бесплатном <Link href="/" className="text-[#163A5F] underline underline-offset-2 hover:text-[#1E4A78]">калькуляторе маркировки</Link>.',
  },
  {
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-start gap-3">
                ...
                <span className="text-sm text-slate-600 leading-relaxed">
                  ...
                  ФН-1.2 и перерегистрируем ККТ в ФНС.
                </span>
              </li>
              <li className="flex items-start gap-3">
